...
async function main() {
  ...
  if (candidates.length === 1) {
    ...
    if (candidate.paths.length === 1 && candidate.paths[0] !== '.') {
      // Exactly one matching project - safe to scope the agent's whole working directory to it.
...
Use the provided tools to explore only this scope and apply a focused refactor. Do not touch files outside this scope.`,
      ...
      const pathsBlock = pathsList.length
        ? `\n\nThe knowledge base matched several likely-relevant project paths for this ticket:\n${pathsList.join('\n')}\n\nThis ticket may apply to only one of these, or to several of them at once (e.g. the same pattern
cleanup duplicated across libs). Apply a focused refactor inside every path the ticket genuinely applies to, and
leave unrelated parts of the repository untouched.`
        : '';
      result = await runAgentLoop({
