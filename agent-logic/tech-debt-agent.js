...
async function main() {
  ...
  if (candidates.length === 1) {
    ...
    console.log(`Running tech-debt agent for ${TICKET_KEY} on ${chosenRepo} (likely area: ${candidate.path})`);
    ...
    // RAG only identifies the repo - which file(s)/project(s) to actually touch is for the agent
    // to work out from the ticket description and the real checked-out code, not a path we compute
    // and lock it into ahead of time. The RAG match is passed along only as a non-binding hint.
    const areaHint =
      candidate.path && candidate.path !== '.'
        ? ` The knowledge base's closest match for this ticket was "${candidate.path}", but confirm that
from the actual code and ticket description - the real scope may be narrower, wider, or elsewhere entirely.`
        : '';

    result = await runAgentLoop({
      rootDir: candidateDirs[chosenRepo],
      systemPrompt: `You are an autonomous senior software engineer paying down tech debt in ${chosenRepo}.
Use the provided tools to explore the codebase, identify exactly what this ticket is asking for, and apply a focused
refactor. Do not make unrelated changes.${areaHint}`,
...
applies to, and leave every other candidate completely untouched.`,
  ...
  for (const candidate of candidates) {
    ...
    const prUrl = await openPullRequest({
      ...
      body: `${result.prSummary}\n\nAutomated tech-debt cleanup generated by agent-hub for ${TICKET_KEY}, scoped to \`${candidate.path}\`.`,
    });
  ...
  const prSummaryLines = openedPrs.map((p) => `- ${p.repo} (${p.path}): ${p.prUrl}`).join('\n');
